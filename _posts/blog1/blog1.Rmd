...
author:
  - name: Siyi Li
    url: https://nifty-siyili-5523.netlify.app/posts/blog1/
date: 09-02-2021
categories: 
  - Covid19
output:
...
```{r,echo =FALSE,message = FALSE, warning = FALSE}
NEW%>%select(year,month, active_cum)%>%
  group_by(year, month)%>%
...
```{r trend,echo =FALSE,message = FALSE, warning = FALSE, fig.cap="Active case and trends during these months"}
# final_data%>%
#   select(year:total_active,active_trend)->trend_data

datatable(final_data, escape = FALSE,
          options = list(paging = FALSE,
                         dom = 'ft',
                         initComplete = JS(
    "function(settings, json) {",
...
The graph \@ref(fig:trend) shows the trend of the active cases in each month, and the table \@ref(tab:stat) indicates the summary for death rate, recovered rate and active cases.
